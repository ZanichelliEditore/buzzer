...
export default defineComponent({
  ...
  setup() {
    ...
    const { orderBy, pagination, query, changePage, makePagination } = useTableQuery(
      failedJobsStore.getFailedJobs
    );
    const { alertData, openAlertModal } = useAlertModal();

    const { failedJobs, loadingObjects, meta, links } = storeToRefs(failedJobsStore);
  ...
  methods: {
    ...
    async getFailedJobs() {
      await this.failedJobsStore.getFailedJobs(this.query).catch(error => {
